...
5. (측정값 선택) 계산필드 우클릭 후 **'복제'** - 복제된 계산필드 이름 : **'측정값 단위'** 로 변경 - 우클릭 후 **'편집'** - 계산식 수정
```
CASE [측정값 선택하기]
WHEN '매출' THEN '원'
WHEN '수량' THEN '개'
...
6. '마크-레이블'에 (측정값 선택), (측정값 단위) 드랍 - **'마크-레이블'** 클릭 후 **'텍스트-...'** 클릭 - 레이블편집 : **<집계(측정값 선택)><측정값 단위>**  
<img width="655" alt="image" src="https://github.com/Choe-minsung/TIL/assets/145301343/7a6d14b9-6e8c-4bf0-9862-10ccd71b44e7">  

- *매개변수 활용 색상 구분 참조선*  
1. '열'에 (매출), '행'에 (국가/지역) 드랍 - 좌상단 '분석'탭에 참조선 드래그, 시트에 **'테이블'** 에 드랍  
2. **'라인-값-새 매개 변수 만들기'** 클릭 - **매개변수 명 : 목표매출** 입력 후 확인  
<img width="281" alt="image" src="https://github.com/Choe-minsung/TIL/assets/145301343/bdd764a8-02a5-45cf-ab75-1486553267e7">  

3. 데이터패널에서 **'계산된 필드 만들기'** - **필드명 : KPI, 계산식 :**  
```
IF SUM([매출]) >= [목표매출]
THEN '목표 달성'
ELSE '목표 미달성'
END
```
4. 만든 계산필드 KPI를 **'마크-색상'** 에 드랍 - 목표매출 숫자 바꿔가며 색상 변화 확인  
<img width="1119" alt="image" src="https://github.com/Choe-minsung/TIL/assets/145301343/85ee79b0-acab-42cc-852a-91c838292bcf">  

- *매개변수 활용 <시작날짜> ~ <마지막날짜> 사이 매출합계*  
1. '열'에 (주문날짜/연속형 월), '행'에 (매출) 드랍   
2. 데이터패널에서 **'매개 변수 만들기'** - **'이름 : 시작날짜, 데이터유형 : 날짜** - 만든 매개변수 **'복제'** 후 '이름 : **마지막날짜**' 로 수정 - 두 매개변수 우클릭 **'매개 변수 표시'**  
3. 좌상단 **'분석'** 탭에서 **'참조 구간'** 드래그, 시트에 **'테이블-월(주문 날짜)'** 에 드랍 - **'시작 구간-값-<시작날짜>'**, **'끝 구간-값-<마지막날짜>'**, **'서식 지정-라인-1번째'** 선택 후 확인  
<img width="280" alt="image" src="https://github.com/Choe-minsung/TIL/assets/145301343/8316e5b2-dfe4-46fc-82ea-2d3a546090d7">  

4. 데이터패널에 **'계산된 필드 만들기'** - **필드명 : 날짜 범위의 매출합계, 계산식 :**  
```
TOTAL(SUM(IF [주문 날짜] >= [시작날짜] AND [주문 날짜] <= [마지막날짜] THEN [매출] END))
```
5. 만든 계산필드를 **'마크-세부정보'** 에 드랍 - 시트제목 : **'<매개 변수.시작날짜>부터 <매개 변수.마지막날짜>
매출합계 : <집계(날짜 범위의 매출 합계)>'** 으로 변경  
<img width="1135" alt="image" src="https://github.com/Choe-minsung/TIL/assets/145301343/083e063f-cb10-45e8-9f1d-ca180e14e0b7">  
