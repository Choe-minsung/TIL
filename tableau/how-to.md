...
1. 매출 히스토그램 : '열'에 (매출) 드랍 - 우상단 탭 **'표현방식-히스토그램 뷰'** - '열'에 (매출)이 **연속형 구간차원**인지, '행'에 (매출)이 **카운트** 인지 확인 - 테이블에 자동 생성 된 **매출(구간차원) 우클릭** 후 **'편집-구간차원 크기'** (눈금) 조절 - y축 최댓값과 최솟값의 차이가 극단적일 때 시트에 y축을 우클릭 후 **'축 편집-눈금-로그'** 체크
